...
class TableSceneBuilder(SceneBuilder):
    ...
    def initialize(self, env_idx: torch.Tensor):
        ...
        if self.env.robot_uids == "panda":
        ...
        elif self.env.robot_uids == "a1_galaxea":
            # Use the home keyframe which has proper joint angles from simple_task project
            ...
            # Keep gripper fingers at their home position (open)
            qpos[:, -2:] = self.env.agent.keyframes["home"].qpos[-2:]
            self.env.agent.reset(qpos)
            self.env.agent.robot.set_pose(sapien.Pose([-0.615, 0, 0]))
        else:
            # Default case - robot will use its default initialization
            pass
