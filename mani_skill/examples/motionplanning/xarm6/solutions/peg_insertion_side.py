...
def main():
    env: PegInsertionSideEnv = gym.make(
        ...
        reward_mode="sparse",  # Use sparse since dense is not supported
    )
...
def solve(env: PegInsertionSideEnv, seed=None, debug=False, vis=False):
    ...
    planner = planner_cls(
        ...
        joint_vel_limits=0.5,  # More conservative limits for precision task
    ...
    res = planner.move_to_pose_with_RRTStar(pre_insert_pose)
    if res == -1:
        return res
