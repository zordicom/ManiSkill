...
from mani_skill.examples.motionplanning.xarm6.solutions import (
...
def _main(args, proc_id: int = 0, start_seed: int = 0) -> str:
    ...
    env = gym.make(
        ...
        reward_mode="sparse"
        if env_id in ["PegInsertionSide-v1", "PlugCharger-v1"]
        and args.reward_mode is None
        else ("dense" if args.reward_mode is None else args.reward_mode),
        sensor_configs=dict(shader_pack=args.shader),
        human_render_camera_configs=dict(shader_pack=args.shader),
        viewer_camera_configs=dict(shader_pack=args.shader),
        sim_backend=args.sim_backend,
        enable_shadow=False,  # Disable shadows to avoid lighting issues
    )
    ...
    while True:
        ...
        if args.only_count_success and not success:
            ...
            if (
                solution_episode_lengths
            ):  # Only add episode length stats if we have successful episodes
                postfix_dict.update(
                    dict(
